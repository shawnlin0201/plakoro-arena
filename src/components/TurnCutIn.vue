  ...
  <div class="turn-cutin" :class="cutIn.key === 'A' ? 'cutin-a' : 'cutin-b'">
    ...
    <div class="turn-cutin-line2">
      <template v-if="cutIn.hasChara">
        <div class="tc-icon"><img :src="asset('image/ICON/キャラコロ.png')" class="img-icon" alt="キャラコロ"></div>
        <span v-if="cutIn.energyCount > 0" class="tc-flow"></span>
      </template>
      <div v-for="(num, i) in energyIcons" :key="i" class="tc-icon"><img :src="asset(`image/ICON/エネコロ${num}.png`)" class="img-icon" alt="エネコロ"></div>
    </div>
  </div>
</template>
